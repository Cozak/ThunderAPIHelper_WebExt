...
struct DownloadInfo
{
	...
	DownloadInfo() : agentName(NULL), count(0), urls(NULL), descs(NULL), cookies(NULL), cids(NULL)
	{
...
protected:
	...
	{
		USES_CONVERSION;
		COMCALL(lpTDispatch.CoCreateInstance(A2OLE(progId ? progId : getProgId()), NULL, CLSCTX_ALL));
		comObj = lpTDispatch;
	}
